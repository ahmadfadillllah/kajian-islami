<footer class="footer">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-6 footer-copyright">
                <p class="mb-0">Copyright 2025 </p>
            </div>
            <div class="col-md-6">
                <p class="float-end mb-0">© {{ config('app.name') }}.

                </p>
            </div>
        </div>
    </div>
</footer>
</div>
</div>
<!-- jquery-->
<script src="{{ asset('dashboard/assets') }}/js/vendors/jquery/jquery.min.js"></script>
<!-- bootstrap js-->
<script src="{{ asset('dashboard/assets') }}/js/vendors/bootstrap/dist/js/bootstrap.bundle.min.js" defer=""></script>
<script src="{{ asset('dashboard/assets') }}/js/vendors/bootstrap/dist/js/popper.min.js" defer=""></script>
<!--fontawesome-->
<script src="{{ asset('dashboard/assets') }}/js/vendors/font-awesome/fontawesome-min.js"></script>
<!-- feather-->
<script src="{{ asset('dashboard/assets') }}/js/vendors/feather-icon/feather.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/vendors/feather-icon/custom-script.js"></script>
<!-- sidebar -->
<script src="{{ asset('dashboard/assets') }}/js/sidebar.js"></script>
<!-- toasts-->
<script src="{{ asset('dashboard/assets') }}/js/toasts-custom.js"></script>
<!-- select2-->
<script src="{{ asset('dashboard/assets') }}/js/select2/select2.full.min.js"></script>
<!-- page_select2-->
<script src="{{ asset('dashboard/assets') }}/js/select2/select2-custom.js"></script>
<!-- height_equal-->
<script src="{{ asset('dashboard/assets') }}/js/height-equal.js"></script>
<!-- config-->
<script src="{{ asset('dashboard/assets') }}/js/config.js"></script>
<!-- apex-->
<script src="{{ asset('dashboard/assets') }}/js/chart/apex-chart/apex-chart.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/chart/apex-chart/stock-prices.js"></script>
<!-- page_apex-->
<script src="{{ asset('dashboard/assets') }}/js/chart/apex-chart/chart-custom.js"></script>
<!-- scrollbar-->
<script src="{{ asset('dashboard/assets') }}/js/scrollbar/simplebar.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/bookmark/jquery.validate.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/bookmark/custom.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/scrollbar/custom.js"></script>
<!-- slick-->
<script src="{{ asset('dashboard/assets') }}/js/slick/slick.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/slick/slick.js"></script>

<!-- photo_swipe-->
<script src="{{ asset('dashboard/assets') }}/js/photoswipe/photoswipe.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/photoswipe/photoswipe-ui-default.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/photoswipe/photoswipe.js"></script>

<!-- select2_options-->
<script src="{{ asset('dashboard/assets') }}/js/vendors/%40yaireo/tagify/dist/tagify.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/vendors/%40yaireo/tagify/dist/tagify.polyfills.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/vendors/%40yaireo/tagify/dist/intlTelInput.min.js"></script>
<!-- page_select3-->
<script src="{{ asset('dashboard/assets') }}/js/select2/select3-custom.js"></script>
<!-- flatpicker-->
<script src="{{ asset('dashboard/assets') }}/js/flat-pickr/flatpickr.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/flat-pickr/custom-flatpickr.js"></script>
<!-- data_table-->
<script src="{{ asset('dashboard/assets') }}/js/js-datatables/datatables/jquery.dataTables.min.js"></script>
<!-- counter-->
<script src="{{ asset('dashboard/assets') }}/js/counter/jquery.waypoints.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/counter/jquery.counterup.min.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/counter/counter-custom.js"></script>
<!-- page_datatable-->
<script src="{{ asset('dashboard/assets') }}/js/js-datatables/datatables/datatable.custom.js"></script>
<!-- page_datatable1-->
<script src="{{ asset('dashboard/assets') }}/js/js-datatables/datatables/datatable.custom1.js"></script>
<!-- page_datatable-->
<script src="{{ asset('dashboard/assets') }}/js/datatable/datatables/datatable.custom.js"></script>
<!-- theme_customizer-->
<script src="{{ asset('dashboard/assets') }}/js/theme-customizer/customizer.js"></script>
<!-- support_ticket_custom-->
<script src="{{ asset('dashboard/assets') }}/js/support-ticket-custom.js"></script>
<!-- tilt-->
<script src="{{ asset('dashboard/assets') }}/js/animation/tilt/tilt.jquery.js"></script>
<!-- page_tilt-->
<script src="{{ asset('dashboard/assets') }}/js/animation/tilt/tilt-custom.js"></script>
<!-- dashboard_1-->
<script src="{{ asset('dashboard/assets') }}/js/dashboard/dashboard_1.js"></script>
<!-- custom_select2-->
<script src="{{ asset('dashboard/assets') }}/js/custom-inputsearch.js"></script>
<script src="{{ asset('dashboard/assets') }}/js/telephone-inpu.js"></script>
<!-- custom script -->
<script src="{{ asset('dashboard/assets') }}/js/script.js"></script>
</body>
</html>
